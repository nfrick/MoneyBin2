...
namespace MoneyBin2 {
    public static class FormUtils {
        public static void ResizeForm(Form frm, DataGridView dgv) {
            frm.Width = 20 + dgv.RowHeadersWidth +
        ...
        public static DialogResult PerguntaSeSalva(MoneyBinEntities ctx, string caption) {
            var alteracoes = ctx.ChangesCount;
            return alteracoes == 0
                ? DialogResult.No
                : MessageBox.Show(TextoSalvar(alteracoes) + @"?", caption, MessageBoxButtons.YesNoCancel,
                    MessageBoxIcon.Question);
        }

        public static void EnableSaveButtons(MoneyBinEntities ctx, ToolStrip menu) {
            var items = menu.Items.Cast<ToolStripItem>()
                .Where(i => (string)i.Tag == "Save");
            foreach (var item in items) {
                item.Visible = ctx.ChangeTracker.HasChanges();
                if (item.Text.StartsWith("Salvar")) {
                    item.Text = TextoSalvar(ctx.ChangesCount);
                }
            }
        }
    }
